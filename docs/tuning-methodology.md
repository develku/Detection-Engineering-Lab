...
Alert tuning is what separates a SOC analyst from someone who just deploys a SIEM.

A detection rule that fires 100 times a day with a 5% true positive rate wastes analyst time and erodes trust in the alert pipeline. In this lab, three rules averaged a 6.5% true positive rate before tuning — analysts would have investigated ~236 false alerts daily. After systematic tuning, false positive volume dropped 73-87% across all three rules without losing a single true positive.

This document covers the methodology behind those results.
...
| Brute Force Detection | 120 | 32 | 73% | 28.1% | [Report](../tuning/brute-force-tuning.md) |
| New Service Created | 85 | 12 | 86% | 25.0% | [Report](../tuning/service-creation-tuning.md) |
